...
// ADD CLASSNAMES 
	if (_faction==5) then {
	_InfPool=	eos_InfPool;
	_ArmPool=	eos_ArmPool;
	_MotPool=	eos_MotPool;
	_ACHPool=	eos_ACHPool;
	_CHPool=	eos_CHPool;
	_uavPool=	eos_uavPool;
	_stPool=	eos_stPool;
	_shipPool=	eos_shipPool;
	_diverPool=	eos_diverPool;
	_crewPool=	eos_crewPool;
	_heliCrew=	eos_heliCrew;
